...
OUTPUT_PATH = os.path.join(DOC_DIR, "ProjectReport_PC_Builder_Pro.docx")
...
def add_markdown_paragraph(doc, text, style=None, align=None, indent_first=False, space_before=0, space_after=6):
    ...
    # Match bold (**bold**) and italic (*italic*)
...
def main():
    print("📖 Starting conversion of markdown reports to formatted Word document...")
    doc = Document()
    ...
    # 1. Read Front Matter (Abstract, Acknowledgements) from report.md
    ...
    if os.path.exists(report_path):
        ...
        recording = False
        for line in lines:
            if "## บทคัดย่อ" in line or "## กิตติกรรมประกาศ" in line:
                recording = True
            if "# บทที่ 1" in line:
                break
            if recording:
    ...
    front_matter_text = "".join(front_matter)
    
    # 2. Define files in order
    files = [
        ("Front Matter", front_matter_text),
        ("Chapter 1", os.path.join(DOC_DIR, "Chapter1_Introduction.md")),
        ("Chapter 2", os.path.join(DOC_DIR, "Chapter2_LiteratureReview.md")),
        ("Chapter 3", os.path.join(DOC_DIR, "Chapter3_Methodology.md")),
        ("Chapter 4", os.path.join(DOC_DIR, "Chapter4_Results.md")),
        ("Chapter 5", os.path.join(DOC_DIR, "Chapter5_Conclusion.md")),
    ]
    ...
    for name, source in files:
        ...
        # Clean guidelines/header notes from chapters
        content = re.sub(r"^# คู่มือการจัดรูปแบบหน้ากระดาษ.*?\n---\n", "", content, flags=re.DOTALL | re.IGNORECASE)
        content = re.sub(r"^# โครงสร้างและรูปแบบเล่มรายงาน.*?\n---\n", "", content, flags=re.DOTALL | re.IGNORECASE)
        ...
        for line in lines:
            ...
            if striped_line.startswith("# ") or striped_line.startswith("## "):
                ...
                # Check for chapter start to add page break
            ...
            elif "ภาพที่" in striped_line and ("**ภาพที่" in striped_line or "ภาพที่ 3." in striped_line or "ภาพที่ 2." in striped_line):
                # Handle figure insertion before the caption
...
def render_word_table(doc, table_lines):
    ...
    for line in table_lines:
        # Split and filter empty cells
        cells = [c.strip() for c in line.split("|")]
        # Remove empty ends due to leading/trailing pipes
    ...
    for r_idx, row_cells in enumerate(rows_data):
        ...
        # Make row header repeat on next page (cantrip)
        ...
        for c_idx, cell_value in enumerate(row_cells):
            if c_idx < len(row.cells):
                ...
                # Alignment based on header or index
